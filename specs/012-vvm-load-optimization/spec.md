...
- Q: モデルのロードは xml-tts 実行時に1度のみ行われる？ → A: はい。起動時に1回ロードし、必要な VVM のみに絞る（遅延ロードは採用しない）
- Q: style_id と VVM ファイルのマッピング方法は？ → A: 静的マッピング（コード内に style_id → VVM ファイルの対応表を定義）
- Q: バージョン不一致の解消方法は？ → A: VVM ファイル再取得（Core バージョンに合う VVM をダウンロード）
...
- **FR-001**: システムは、起動時に指定された style_id に対応する VVM ファイルのみをロードしなければならない（遅延ロードは採用しない）
- **FR-002**: システムは、style_id から対応する VVM ファイルを特定する静的マッピング（コード内定義）を提供しなければならない
...
- style_id と VVM ファイルのマッピングは、VOICEVOX 公式仕様に基づく静的定義として実装する
- VVM ファイルを VOICEVOX Core バージョンに合わせて再取得することで、バージョン不一致は解消可能
