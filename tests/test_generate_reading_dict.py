...
class TestMdInputUsesSplitIntoPages:
    ...
    def test_md_input_calls_split_into_pages(
        ...
        with patch("src.generate_reading_dict.split_into_pages") as mock_split, \
             patch("sys.argv", ["prog", str(md_file)]):
            ...
            assert mock_extract_terms.call_count == 2, (
                f"extract_technical_terms should be called once per page (2 pages), "
                f"got {mock_extract_terms.call_count}"
            )
...
class TestUnsupportedExtensionError:
    ...
    def test_unsupported_extension_logs_error_message(self, tmp_path):
        ...
        with patch("sys.argv", ["prog", str(txt_file)]), \
            ...
            assert "Unsupported" in error_msg or "unsupported" in error_msg.lower() or \
                   ".xml" in str(mock_logger.error.call_args) or \
                   ".md" in str(mock_logger.error.call_args), (
...
class TestEmptyXmlGeneratesEmptyDict:
    ...
    def test_empty_xml_no_terms_extracted(
        ...
        with patch("src.generate_reading_dict.parse_book2_xml", return_value=[]), \
             patch("src.generate_reading_dict.extract_technical_terms") as mock_extract, \
...
class TestInvalidXmlErrorExit:
    ...
    def test_malformed_xml_exits_with_error(self, tmp_path):
        ...
        with patch("sys.argv", ["prog", str(DICT_TEST_INVALID_XML)]):
            ...
            assert exc_info.value.code == 1, (
                f"Expected exit code 1 for malformed XML, got {exc_info.value.code}"
            )
    ...
    def test_xml_parse_error_caught_gracefully(self, tmp_path):
        ...
        with patch("src.generate_reading_dict.parse_book2_xml") as mock_parse, \
             patch("sys.argv", ["prog", str(DICT_TEST_BOOK_XML)]), \
             patch("src.generate_reading_dict.get_dict_path") as mock_get_path, \
            ...
            assert exc_info.value.code == 1, (
                f"Expected exit code 1 for XML parse error, got {exc_info.value.code}"
            )
...
class TestChapterNumberNoneContentItem:
    ...
    def test_none_chapter_items_are_included_in_extraction(
        ...
        items = [
            ContentItem(item_type="paragraph", text="API design patterns",
                        chapter_number=None),
            ContentItem(item_type="paragraph", text="REST architecture",
                        chapter_number=1),
        ]
    ...
    def test_none_chapter_terms_included_in_final_result(
        ...
        items = [
            ContentItem(item_type="paragraph", text="GraphQL is a query language",
                        chapter_number=None),
            ContentItem(item_type="paragraph", text="Docker containers",
                        chapter_number=1),
        ]
        ...
        with patch("src.generate_reading_dict.parse_book2_xml", return_value=items), \
            ...
            assert "GraphQL" in call_args, (
                "Terms from chapter_number=None items should be included"
            )
            assert "Docker" in call_args, (
                "Terms from numbered chapters should also be included"
            )
    ...
    def test_all_none_chapter_items_grouped_together(
        ...
        items = [
            ContentItem(item_type="paragraph", text="First ungrouped item about API",
                        chapter_number=None),
            ContentItem(item_type="paragraph", text="Second ungrouped item about REST",
                        chapter_number=None),
        ]
        ...
        with patch("src.generate_reading_dict.parse_book2_xml", return_value=items), \
            ...
            assert "First ungrouped item about API" in call_text, (
                "Combined text should contain first None-chapter item"
            )
