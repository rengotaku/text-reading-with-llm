...
class TestPyprojectTomlRuffSettings:
    ...
    def test_ruff_line_length_is_120(self, pyproject_content):
        ...
        assert "line-length = 120" in pyproject_content, "pyproject.toml に line-length = 120 が設定されていない"
    ...
    def test_ruff_lint_select_contains_required_rules(self, pyproject_content):
        ...
        assert "[tool.ruff.lint]" in pyproject_content, "pyproject.toml に [tool.ruff.lint] セクションがない"
        # select に必要なルールが全て含まれていることを確認
        for rule in ["E", "F", "I", "W"]:
            assert f'"{rule}"' in pyproject_content, f"pyproject.toml の ruff lint select に {rule} が含まれていない"
    ...
    def test_ruff_isort_known_first_party(self, pyproject_content):
        ...
        assert '"src"' in pyproject_content, 'pyproject.toml の isort known-first-party に "src" が含まれていない'
...
class TestPreCommitConfigYaml:
    ...
    def test_precommit_has_ruff_hook_with_fix(self, precommit_content):
        ...
        assert "id: ruff" in precommit_content, ".pre-commit-config.yaml に ruff フックが設定されていない"
    ...
    def test_precommit_has_ruff_format_hook(self, precommit_content):
        ...
        assert "id: ruff-format" in precommit_content, ".pre-commit-config.yaml に ruff-format フックが設定されていない"

    def test_precommit_has_version_pinned(self, precommit_content):
        """ruff-pre-commit のバージョンが固定されていることを確認する。"""
        assert "rev:" in precommit_content, ".pre-commit-config.yaml でバージョンが固定されていない（rev が未設定）"
