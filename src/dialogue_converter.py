...
def generate_dialogue(
    ...
    prompt = f"""以下のテキストを、博士（A）と助手（B）の楽しい対話形式に変換してください。

{full_context}

【キャラクター設定】
- A（博士）: ベテランエンジニア。説明がうまく、身近な例えを使う。「〜なんだよ」「〜だね」口調。
- B（助手）: 若手エンジニア。好奇心旺盛で素直なリアクション。「へー！」「なるほど！」「それってつまり〜ですか？」

【変換ルール】
- ポッドキャスト番組のような親しみやすい会話
- 専門用語は身近な例えで説明する
- Bは素朴な疑問を投げかけ、Aがわかりやすく答える
- 「なるほど」「面白いですね」などリアクションを入れる
- A→B→A→B...の順番で交互に発話
- 堅すぎる敬語は避け、自然な話し言葉で
